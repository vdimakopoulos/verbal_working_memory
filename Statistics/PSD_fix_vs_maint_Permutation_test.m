...
for iSS = 1:nSet_Size
    cfg = [];
    cfg.latency = [-2,0-1/dataBipolar_SS{iSS}.fsample];
...
for iChannel = ind(1):ind(end)%1:length(nChannelList)
    ...
    %% Save results
    iSS = 5;
    save([strVariableFolder,'Patient_',num2str(42),'_Pair_',num2str(nChannel,'%.4d'),'_',dataBipolar_Fix_SS{iSS}.label{nChannel},'_mt_tsf_2_Fix_vs_Maint_PSD',num2str(2*iSS+2,'%.1d'),'.mat'],...
    ...
    %% Save stats
    strPaths.stats = [strPaths.Results,'Statistics\Rand_Stats\PSD_fix_vs_maintenance\']
    mkdir(strPaths.stats)
    save([strPaths.stats,'Patient_',num2str(42),'_Pair_',num2str(nChannel,'%.4d'),'_',dataBipolar_Fix_SS{iSS}.label{nChannel},'_mt_tsf_2_Stats_Summary_Across_Periods_Fix_vs_Maintenance_PSD','.mat'],...
...
for iChannel = ind(1):ind(end)
    nChannel = iChannel;
    iSS = 4;
    strPaths.stats = 'F:\Vasileios\Task Analysis\Extracted_Data Sternberg\Statistics\Rand_Stats\PSD_fix_vs_maintenance\';
    StatsVars = load([strPaths.stats,'Patient_',num2str(42),'_Pair_',num2str(nChannel,'%.4d'),'_',dataBipolar_Fix_SS{iSS}.label{nChannel},'_mt_tsf_2_Stats_Summary_Across_Periods_Fix_vs_Maintenance_PSD','.mat']);
    boolLinearScale = 0 ;
    PSD_maint_vector(iChannel,:) = mean(StatsVars.PSD_Rand_Dist_SingleSS(:,:),2)
% end
    if boolLinearScale
...
%     axes(ha(plot_order(iChannel-8)))
    ...
    semilogx(StatsVars.freqAxis,Real_TP2,'r','LineWidth',3);
    hold on;
...
%     if PSD_In_Band_SS(iChannel-8)<1.3
%         PSD_In_Band_SS(iChannel-8) = 0
%     end
end
PSD_In_Band_ToPlot = reshape(PSD_In_Band_SS,8,8);
%%
figure;
imagesc(PSD_PLV_Granger.PSD_topo_enc,[2 4])
