...
if Granger_significance
    ...
    save([strVariableFolder,'Patient_',num2str(42,'%.2d '),'_','Grid-Hippocampus','_Randomized_Trial_Numbers_200_perms.mat'],'nRandomTrialNumberList_Pair_SS','nNumberOfPermutations','-v7.3')
    %% Channel Pairs
...
%         TimeInterval = [-2,-1/dataBipolar_Ret_SS{iSS}.fsample];
        ...
        mkdir(strPaths.var)
        save([strPaths.var ,'Patient_',num2str(42,'%.2d'),'_Rand_Granger_Pair_',num2str(nPair,'%.4d'),'_',dataBipolar_Ret_SS{iSS}.label{nChannel_1},'_',dataBipolar_Ret_SS{iSS}.label{nChannel_2},'_mt_tsf_2_ss',num2str(iSS),'.mat'],...
...
%         Granger_Rand_Dist_SingleSS = abs(Granger_Rand_Dist_SingleSS{1})-abs(Granger_Rand_Dist_SingleSS{2});
        ...
        save([strPaths.stats,'Rand_Stats_Patient_',num2str(42,'%.2d'),'_Rand_Granger_Pair_',num2str(nPair,'%.4d'),'_',dataBipolar_Ret_SS{iSS}.label{nChannel_1},'_',dataBipolar_Ret_SS{iSS}.label{nChannel_2},'_mt_tsf_2_Stats_ss',num2str(2*iSS+2),'.mat'],...
...
%         clear gdata_Pairs_ss
    ...
    %% Load results / Granger for all pairs / Randomized trials / 2 seconds of retention
    ...
    for iSS = iSS_ToCompute%1:size(dataBipolar_Ret_SS,2)
        ...
        for nPair = 1:size(nChannelPairs,1)
            nChannel_1 = nChannelPairs(nPair,1);
            nChannel_2 = nChannelPairs(nPair,2);
            strVariablePath = [strPaths.stats,'Rand_Stats_Patient_',num2str(42,'%.2d'),'_Rand_Granger_Pair_',num2str(nPair,'%.4d'),'_',dataBipolar_Ret_SS{iSS}.label{nChannel_1},'_',dataBipolar_Ret_SS{iSS}.label{nChannel_2},'_mt_tsf_2_Stats_ss',num2str(2*iSS+2),'.mat'];
            try
    ...
    %% Plot the Granger spectra with significance for the grid hippocampus
    
    %Load the granger real values
    ...
    indMaxBandCortexHipp_Enc = Difference_Bar(Grngr_Enc.grangerspctrm(1,:)-Vars.RandPrc{99},Vars.RandPrc{99},freq,[0 0.2], 0.005,Colors{1});
